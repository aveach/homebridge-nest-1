...
function NestThermostatAccessory(conn, log, device, structure, platform) {
  ...
  if (this.platform.shouldEnableFeature("Thermostat.Heat")) {
    const thermostatHeatModeService = this.addService(Service.Switch, "Heat", "heat_mode");
    this.bindCharacteristic(thermostatHeatModeService, Characteristic.On, "Heat", this.getHeatMode, this.setHeatMode);
  }
...
NestThermostatAccessory.prototype.getCoolingThresholdTemperature = function () {
  ...
  switch (this.device.hvac_mode) {
  case "eco":
  this.log("in getCoolingThresholdTemperature eco block")
    // away_temperature deprecated in v5. in v6 use eco_temperature but if undefined, fallback to away_temperature
    ret_val = this.getTemperatureValueInCelsius('eco_temperature_high_') || this.getTemperatureValueInCelsius('away_temperature_high_');
    this.log(ret_val);
    return ret_val;
  case "heat-cool":
  default:
    this.log("in getCoolingThresholdTemperature default block")
    ret_val = this.getTemperatureValueInCelsius('target_temperature_high_')
    this.log(ret_val);
    return ret_val;
  }
};
...
NestThermostatAccessory.prototype.setTargetTemperatureDebounced = debounce(function (targetTemperature, callback) {
  ...
  let currentMode = this.device.hvac_mode;
  let promise = Promise.resolve();

  this.log("This is ambient: " + ambient);
  this.log("Current Mode is: " + currentMode);

  if (currentMode == 'eco' || currentMode =='off') {
    if (targetTemperature > ambient) {
      currentMode = 'heat';
    }
    if (targetTemperature < ambient) {
      currentMode = 'cool';
    }
    this.log("First setting thermostat to: " + currentMode);
    promise = promise.then(() => this.updateDevicePropertyAsync("hvac_mode", currentMode, "target heating cooling"));
  } else if (currentMode == 'heat-cool') {
      const targetHigh = this.getCoolingThresholdTemperature();
      const targetLow = this.getHeatingThresholdTemperature();

      this.log("targetLow: " + targetLow + "targetHigh: " + targetHigh + "targetTemperature: " + targetTemperature);
  ...
  this.log("setting: " + setting + " targetTemperature: " + targetTemperature)
  sleep(2000).then(() => {
...
NestThermostatAccessory.prototype.getHeatMode = function () {
  return (this.device.hvac_mode === "heat");
};

NestThermostatAccessory.prototype.setHeatMode = function (heat, callback) {
  const val = heat ? 'heat' : 'eco';
  this.log.info("Setting Heat Mode for " + this.name + " to: " + val);
  return this.updateDevicePropertyAsync("hvac_mode", val, "target heating cooling")
    .asCallback(callback);
};
